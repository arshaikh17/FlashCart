...
<div class="box box-default">
    ...
    <div class="box-body">
        <form action="/store/product/display/" class="form-horizontal" role="form" method="POST" >
        {{csrf_field()}}
            <div class="form-group">
            <input type="text" class="hidden" name="product_id" value="{{ $product['id'] }}" />
            <input type="text" class="hidden" name="product_display" value="@if($product['display'] == 0) 1 @elseif($product['display'] == 1) 0 @endif" />
                <div class="col-lg-3 col-lg-offset-4">
                <label class="">Display product on main page: </label> &nbsp;&nbsp;
                    <input onChange="this.form.submit()" type="checkbox" name="check_display" @if($product['display'] == 1) checked @endif />
                </div>
            </div>
        </form>
